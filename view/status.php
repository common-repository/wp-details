...
<div class="bsi"> 
    <h1 class="wp-heading-inline" id="wordpress">WP details</h1>

	<ul>
  <li><a href="#wordpress">WordPress information</a></li>
  <li><a href="#server">Server status</a></li>
  <li><a href="#theme">Active Theme</a></li>
  <li><a href="#plugins">Active Plugins</a></li>
</ul>
<div>	
    ...
    <table class="" cellspacing="0" id="data-d">
        ...
        <tr>
            <td width="25%"><?php _e('Homepage', 'bsi'); ?>:</td>
...
<td><?php form_option('home'); ?></td>
        ...
        <tr>
            ...
            <td><?php
                ...
                if ($memory < 67108864) {
                    echo '<span class="warning"><span class="dashicons dashicons-warning"></span> ' . sprintf(__('%s - For better performance, we recommend setting memory limit to at least 64MB. See: %s', 'bsi'), size_format($memory), '<a href="https://codex.wordpress.org/Editing_wp-config.php#Increasing_memory_allocated_to_PHP" target="_blank">' . __('Increasing memory allocated to PHP', 'bsi') . '</a>') . '</span>';
                } else {
        ...
        <tr>
            <td  ><?php _e('Upload Directory', 'bsi'); ?>:</td>
        ...
        <?php if (function_exists('ini_get')) : ?>
            ...
            <tr>
                <td  ><?php _e('GD Library', 'bsi'); ?>:</td>

                <td><?php $gdl = gd_info()  ; 
                    if($gdl) echo "Yes, version: ". $gdl['GD Version'];else echo "No";
                ?></td>
            </tr>
        ...
        <tr>
            ...
            <td><?php
                echo is_child_theme() ? '<span class="yes"><span class="dashicons dashicons-yes"></span>Yes</span>' : '<span class="dashicons dashicons-no-alt"></span> No.  ' . sprintf(__('If you want to modify a theme, it\'s safer to create a child theme.  See: <a href="%s" target="_blank">How to create a child theme</a>', 'bsi'), 'https://codex.wordpress.org/Child_Themes');
                ?></td>
        </tr>
        <?php
        if (is_child_theme()) :
	...
	<a href="javascript:emailCurrentPage()" class="export-wsi-data button button-primary button-large">Email</a>
    <a href="JavaScript:void(0)" class="export-wsi-data button button-primary button-large"> Export to CSV</a>
	
	<!--<p>To make this information public, create a New page/post and add this shortcode: <div class="tooltip"><input type="text" value="[wp_details]" id="myInput">

<button onclick="myFunction()" onmouseout="outFunc()">
  <span class="tooltiptext" id="myTooltip">Copy shortcode</span>
  Copy
  </button>
</div>
	</p>-->
